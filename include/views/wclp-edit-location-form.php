					...
					<tr valign="top">
						<td>
							<h3 style="">
								...
								<a href="<?php echo admin_url().'admin.php?page=local_pickup&tab=locations';  ?>" class="link decoration"><?php _e( 'Pickup Locations', 'advanced-local-pickup-for-woocommerce' ); ?></a> > 
								<?php } ?>
								<?php _e( 'Edit Pickup Location', 'advanced-local-pickup-for-woocommerce' ); ?></h3>
						</td>
					</tr>
				</tbody>
			</table>
						...
						foreach($new_array as $key => $data){
							if(count($data) == 1){							
								if(isset(reset($data)['wclp_store_hour']) && reset($data)['wclp_store_hour'] != '' && isset(reset($data)['wclp_store_hour_end']) && reset($data)['wclp_store_hour_end'] != ''){
									reset($data);
									?>		
									<?php if($key != 1 ){echo '';}echo __(substr(ucfirst(key($data)),0, 3), 'default'); ?><span>: <?php echo reset($data)['wclp_store_hour'].''; echo '-'; echo reset($data)['wclp_store_hour_end']; ?></span>	
							<?php } } ?>						
							<?php
							if(count($data) == 2){
								if(isset(reset($data)['wclp_store_hour']) && reset($data)['wclp_store_hour'] != '' && isset(reset($data)['wclp_store_hour_end']) && reset($data)['wclp_store_hour_end'] != ''){
								...
								?>
									<?php if($key != 1 ){echo '';}echo __(ucfirst($array_key_first), 'default'); ?><span>-</span><?php echo __(ucfirst($array_key_last), 'default'); ?><span>: <?php echo reset($data)['wclp_store_hour'].''; echo '-'; echo reset($data)['wclp_store_hour_end']; ?></span>
							<?php } } ?>									
							<?php 
							if(count($data) > 2){ 
								if(isset(reset($data)['wclp_store_hour']) && reset($data)['wclp_store_hour'] != '' && isset(reset($data)['wclp_store_hour_end']) && reset($data)['wclp_store_hour_end'] != ''){
								...
								$array_key_last = substr(key($data),0, 3);		
								?>
									<?php if($key != 1 ){echo '';}echo __(ucfirst($array_key_first), 'default'); ?> <?php echo __(' to', 'advanced-local-pickup-for-woocommerce'); ?> <?php echo __(ucfirst($array_key_last), 'default'); ?><span>: <?php echo reset($data)['wclp_store_hour'].''; echo '-'; echo reset($data)['wclp_store_hour_end']; ?></span>	
							<?php 										
							} }	
						}
					}
