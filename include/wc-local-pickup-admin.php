...
class WC_Local_Pickup_admin {
	...
	*/
	...
	function add_addional_content_on_processing_email($order, $sent_to_admin, $plain_text, $email){
		...
		echo '<p>'._e(stripslashes($addional_content), 'advanced-local-pickup-for-woocommerce').'</p>';
	}
	...
	function wclp_general_setting_fields_func() {		
		$show_pickup_instraction_option = array( 
			...
			"display_in_order_details_page" => __( 'My Account (orders history)', 'advanced-local-pickup-for-woocommerce' ),			
		);
		$settings = array(						
...
<?php 
	...
	*/
	public function get_pickup_data( $order_id ) {
		...
		if(empty($order_id)) return $location_data = array();
	...
	*/
	public function get_store_address_by_id( $location_id ) {
		...
		if( empty( $location_id ) ) return $address = '';
	...
	*/
	public function get_store_hours_by_id( $location_id ) {
		...
		if( empty( $location_id ) ) return $pickup_hour = '';
		...
		foreach($new_array as $key => $data){
			...
			if(count($data) == 1){	
				...
				if(isset(reset($data)['wclp_store_hour']) && reset($data)['wclp_store_hour'] != '' && isset(reset($data)['wclp_store_hour_end']) && reset($data)['wclp_store_hour_end'] != ''){
					reset($data);
					$pickup_hour .= __(ucfirst(key($data)), 'default').' : ' . reset($data)['wclp_store_hour'].' - ' . reset($data)['wclp_store_hour_end'];
				}
			}
			if(count($data) == 2){
				...
				if(isset(reset($data)['wclp_store_hour']) && reset($data)['wclp_store_hour'] != '' && isset(reset($data)['wclp_store_hour_end']) && reset($data)['wclp_store_hour_end'] != ''){
					...
					$pickup_hour .= __(ucfirst($array_key_first), 'default').' - ' ; $pickup_hour .= __(ucfirst($array_key_last).' ', 'default').' : '. reset($data)['wclp_store_hour'].' - '. reset($data)['wclp_store_hour_end'];
				}
	...
	*/
	public function update_email_preview_order_fun(){
		set_theme_mod('wclp_pickup_preview_order_id', wc_clean($_POST['wclp_preview_order_id']));
		set_theme_mod('wclp_ready_pickup_preview_order_id', wc_clean($_POST['wclp_preview_order_id']));	
		set_theme_mod('wclp_pickup_instruction_preview_order_id', wc_clean($_POST['wclp_preview_order_id']));
		exit;
	}
}