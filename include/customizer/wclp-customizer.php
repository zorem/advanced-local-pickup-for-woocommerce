...
class WC_Local_pickup_Customizer {
	...
	*/
	...
	public function wclp_add_customizer_sections( $wp_customize ) {	
		...
		$wp_customize->add_section( 'pickup_instaruction',
			array(
				'title' => __( 'Pickup Instructions Layout', 'advanced-local-pickup-pro' ),
				'description' => esc_html__( '', 'advanced-local-pickup-pro'  ),
				'panel' => 'wclp_naviation_panel'
			)
		);
		
		$wp_customize->add_section( 'customer_email_notifications',
			array(
				'title' => __( 'Email Notifications', 'advanced-local-pickup-pro' ),
				'description' => esc_html__( '', 'advanced-local-pickup-pro'  ),
				'panel' => 'wclp_naviation_panel'
			)
		);
	...
	*/
	public function enqueue_customizer_scripts(){
		if(isset( $_REQUEST['wclp-customizer'] ) && '1' === $_REQUEST['wclp-customizer']){
			...
			$email_type = isset( $_REQUEST['order_status'] ) ? $_REQUEST['order_status'] : 'ready_pickup';
	...
	*/
	public static function is_own_section( $key ) {
		...
		if ($key === 'wclp_naviation_panel' || $key === 'customer_email_notifications' || $key === 'pickup_instaruction' ) {
			return true;
		}
